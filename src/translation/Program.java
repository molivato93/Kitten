...
public class Program {
	...
	public void cleanUp() {
		...
		// Salvo temporaneamente le signature dei test e delle fixture che ho
		// già trovato, inoltre mi salvo le signature dei costruttori degli oggetti raggiunti
		Set<ClassMemberSignature> temp = new HashSet<ClassMemberSignature>();

		for (ClassMemberSignature cms : sigs)
			if (cms instanceof TestSignature 
					|| cms instanceof FixtureSignature
					|| cms instanceof ConstructorSignature)
				temp.add(cms);

		// Elimino dall'insieme tutte le signature che non sono ne test ne
		// fixture o costruttori
		sigs.retainAll(temp);

		// Pulisco le altre signature e le riassegno
	...
	/**
	 * Generates the Java bytecode for all the class types that have a test and
	 * dumps the relative {@code .class} files on the file system.
	 */

	public void generateJavaBytecodeForTests() {
		// we consider one class at the time and we generate its Test Java bytecode
		for (ClassType clazz: ClassType.getAll())
