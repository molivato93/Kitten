...
public final class ClassType extends ReferenceType {
	...
	/**
	 * A map from method symbols to the set of signatures of the tests with
	 * that name. Because of overloading, more than one method might have a given name.
	 */

	private final Map<String, TestSignature> test = new HashMap<>();

	/**
	 * The set of constructor signatures in this class.
	 */

	private final Set<FixtureSignature> fixture = new HashSet<>();
	...
	/**
	 * Adds a test to this class. If a method with the given name
	 * and signature already existed, it is overwritten.
	 *
	...
	public final void addTest(String name, TestSignature sig) {
		// we add this new test
		test.put(name, sig);
	}
	
	/**
	 * Adds a fixture to this class. If a fixture with the given name
	 * and signature already existed, it is overwritten.
	 *
	 */

	public final void addFixture(FixtureSignature sig) {
		// we add this new fixture
		fixture.add(sig);
	}
	 ...
	 * @return the signature of the method, as defined in this class
	 * 		 Yields {@code null} if no such method has been found
	 */

	public final TestSignature testLookup(String name) {
		// we check all test in this signature having the given name
		return test.get(name);
	}
	
	/**
	 * Ritorno l'insieme di tutte le {@codeTestSignature}
	 * @return l'insieme di tutte le signature
	 */
	public final Set<TestSignature> testsLookup(){
		return new HashSet<TestSignature>(test.values());
	}
